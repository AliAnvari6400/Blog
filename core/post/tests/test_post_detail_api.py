...
class TestPostDetailApi:
    ...
    def test_post_unauthorized_response_401(self, api_client, common_user1):
        ...
        url = reverse("post:api-v1:post-detail", kwargs={"pk": task.id})
    ...
    def test_post_create_by_user(self, api_client, common_user1):
        api_client.force_login(user=common_user1)
        profile = Profile.objects.get(user=common_user1)
        ...
        url = reverse("post:api-v1:post-detail", kwargs={"pk": post.id})
        ...
        # PUT:
        data_put = {"title": "test2", "status": False, "content": "test"}
        response_put = api_client.put(url, data_put)
    ...
    def test_post_not_create_by_user(self, api_client, common_user1, common_user2):
        api_client.force_login(user=common_user2)
        profile = Profile.objects.get(user=common_user1)
        post = Post.objects.create(
            author=profile, title="test", status=True, content="test"
        )
        url = reverse("post:api-v1:post-detail", kwargs={"pk": post.id})
